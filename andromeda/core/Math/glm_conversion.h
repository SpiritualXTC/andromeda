...
namespace andromeda
{
	namespace glmx
	{
		...
		/*
			...
			TODO: These now cause compilation errors in VS2017 :(
			TODO: Remove all teh enums :/
			TODO: Remove all ::detail

			Goddamn GLM changing the library :(
		*/
		...
		struct _Convert<T, P, glm::tvec3>
		{
			typedef glm::tvec3<T, P> TYPE;
		...
		/* 4D */
		template < typename T, enum glm::precision P>
		struct _Convert<T, P, glm::tvec4>
		{
			typedef glm::tvec4<T, P> TYPE;
			...
			// Convert to 2D to 4D
			template <typename _T, enum glm::precision _P = glm::precision::defaultp>
			static const inline TYPE convert(const glm::tvec2<_T, _P> & v, const T def)
			{
			...
			// Convert to 3D to 4D
			template <typename _T, enum glm::precision _P = glm::precision::defaultp>
			static const inline TYPE convert(const glm::tvec3<_T, _P> & v, const T def)
			{
				return TYPE(v, def);
			}
		};
