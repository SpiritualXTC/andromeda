...
// TODO: Implement via the observer
#include "observer.h"
#include "singleton.h"
...
namespace andromeda
{
	...
	class Log : public Singleton<Log>, public Observable<const Log* const, const LogMessage&>
	{
	public:
		enum LogLevel
		{
	...
	private:
		...
		template <typename T>
		const char * argument(const std::shared_ptr<T> & value) noexcept
		{
			return value.get() ? argument(value.get()) : "null";
		}

		template <typename T>
		const char * argument(const T * ptr) noexcept
		{
#if defined(_DEBUG)
	...
	/*
		Automatic Pusher/Popper for structuring heirachy in debug output
	*/
	struct __LogTree
	{
	public:
