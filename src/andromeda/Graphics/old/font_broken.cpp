#include <andromeda/Graphics/Old/font_broken.h>
...
/*
	Broken Font based on Bezier Curves
*/
...
*/
FontBroken::FontBroken(const std::string & font, Int32 size)
{
	// Create the Texture
	...
	_texture = andromeda::LoadTexture("bezier");
...
*/
Boolean FontBroken::loadFont(const std::string & font, Int32 size)
{
	FT_Library lib;
...
*/
Boolean FontBroken::loadChar(const FT_Face & fontFace, ULong charCode)
{
	// Load the Glyph
...
*/
Float FontBroken::calcWidth(const std::string & string)
{
	return -1.0f;
}
...
*/
void FontBroken::drawText(const std::shared_ptr<andromeda::IShader> shader, const std::string & string, andromeda::MatrixStack & ms)
{
	assert(shader);
	assert(_texture);
...
*/
Float FontBroken::drawCharacter(ULong character)
{
	// Find Character
	const auto & it = _characters.find(character);
