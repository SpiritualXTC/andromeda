...
std::shared_ptr<Texture> MeshLoader::loadTexture(const aiMaterial * pmaterial, aiTextureType type, UInt32 index)
{
	...
	log_errp("MeshResource :: loadTexture() :: Loading Textures for meshes is currently disabled");
	std::shared_ptr<Texture> texture = nullptr;// andromeda::LoadTexture(fn.c_str());
