...
namespace andromeda
{
	...
	*/
	...
	/*
		...
		This class needs to do MORE ... (maybe)!
	*/
	class Font
	{
	private:
	...
	public:
		Font(const std::string & ttfFile);
		Font(const std::shared_ptr<File> & file);
		
		virtual ~Font();
		
		// Constant
		Int32 getFontWidth();	// Is NOT affected by Font Size
		Int32 getFontHeight();	// Is NOT affected by Font Size
		...
		Boolean getCharacterOutline(UInt32 charCode, geometry::Shape & shape);
		Boolean getCharacterBitmap(UInt32 charCode, ImageData<UInt8> & image);



		Int32 getLineSpacing();	// Is affected by Font Size
